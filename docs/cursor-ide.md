...
> Cursor Edition overlay: progress fields (`lens_index`/`lens_total`/`last_lens`), stricter False-Green defaults (400 bytes / ≥2 anchors), chat-loop footer, and `run_complete` with `next_action: plan_mode` / batch-grilling. See also [docs/en/handoff.md](en/handoff.md).
...
This Cursor Edition path is **IDE handoff only**. Do not substitute `--agent cursor` (CLI) here.
