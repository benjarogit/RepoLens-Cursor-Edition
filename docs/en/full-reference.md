...
- **Specification files from untrusted sources are dangerous.** Content selected by `--spec` or `--spec-dir` is embedded in the agent prompt. While RepoLens sanitizes known tag-breakout vectors (e.g., `</spec>` injection), a malicious spec document can still influence agent behavior through indirect prompt injection. Only use specification files you wrote or trust completely.
...
Greenfield is a planning mode, not an implementation mode. The product owner puts all human product intent in one `--spec` file or a document bundle selected with `--spec-dir`. RepoLens reads that specification and the current backlog snapshot; it does not inspect repository code, dependencies, configuration, tests, docs, or implementation details to discover work. The greenfield planner turns the spec into implementation-sized `[P0]`-`[P3]` backlog items and resolves remaining non-human product and implementation decisions into the issue details so AutoDev receives decision-complete work.

### Multi-file specifications

Use `--spec-dir` when the product specification is split across several documents:

```bash
./repolens.sh --project ~/my-app --agent cursor-ide --local --mode greenfield \
  --spec-dir ~/docs/product-spec \
  --spec-entry index.md \
  --spec-glob '**/*.md' \
  --spec-exclude 'notes/**,decisions/archive/**' \
  --yes
```

Without `--spec-glob`, RepoLens includes `**/*.md`, including Markdown files at the specification root. Include and exclude patterns are relative to `--spec-dir`; `*` matches within one path segment while a complete `**` segment crosses directories. Both flags may be repeated or receive comma-separated patterns (literal commas in patterns are not supported). Overlapping includes are deduplicated, and exclusions always win.

RepoLens places `--spec-entry` first, then orders the remaining relative paths bytewise and lexicographically for reproducible prompts. The entry must survive the include/exclude filters. Ordering controls presentation only: it does not make one document override another. Generated planning work cites the originating relative file, and conflicting documents are reported with the chosen interpretation instead of being silently resolved.

The selected text and file-boundary markers share the existing 100 KB specification limit. Invalid directories, binary documents, excluded entries, and selections with no matching files fail before agents run. `--spec` and `--spec-dir` are mutually exclusive, and the directory-specific flags are available only in `greenfield` and `spec-change`.

Use `--dry-run` to inspect the canonical root, ordered file list, entry annotation, file count, combined byte size, and manifest path before spending agent calls. Every bundle run persists `spec-files.json` and `combined-spec.md` under `logs/<run-id>/`; its `summary.json` keeps `spec` as `null` and adds a `spec_set` object containing the directory root, manifest path, file count, and combined size.

For `spec-change`, the specification directory must be inside `--project` and every currently selected document must be tracked by Git. RepoLens composes current and base snapshots, diffs the complete bundle, and retains file-boundary paths so edits and deleted documents remain attributable:

```bash
./repolens.sh --project ~/my-app --agent cursor-ide --local --mode spec-change \
  --spec-dir ~/my-app/docs/spec \
  --spec-entry index.md \
  --yes
```
...
| `--spec <file>`        | Single spec/PRD/roadmap file to guide analysis (any text file, max 100 KB). Required for `greenfield` and `spec-change` unless `--spec-dir` is used. The two input forms are mutually exclusive.                                                                                                      |
| `--spec-dir <dir>`     | Root of a multi-file specification for `greenfield` or `spec-change`. Defaults to selecting `**/*.md`; the complete composed bundle, including file markers, is limited to 100 KB. For `spec-change`, the directory must be inside `--project`.                                                        |
| `--spec-glob <pattern>` | Include paths relative to `--spec-dir`. May be repeated or contain comma-separated patterns; overlapping matches are deduplicated. Requires `--spec-dir`.                                                                                                                                           |
...
| `Domain '…' not found in domains.json`                                                                                               | Typo in `--domain` id, or mode mismatch                                                                      | `discover` / `deploy` / `opensource` / `content` / `greenfield` / `polish` modes only see their own domain — see the [Modes](#modes) table                                           |
| `Mode 'custom' requires --change "your change statement"`                                                                            | `--mode custom` without a change statement                                                                   | Pass `--change "your statement"`                                                                                                                                                    |
| `Mode 'greenfield' requires --spec <file> or --spec-dir <dir>`                                                                       | `--mode greenfield` without a product specification                                                          | Pass `--spec path/to/spec.md` or `--spec-dir path/to/specs`                                                                                                                        |
| `No spec files matched in … using pattern(s): …`                                                                                     | The effective `--spec-glob` and `--spec-exclude` filters selected no files                                   | Check patterns relative to `--spec-dir`; omit `--spec-glob` to use the `**/*.md` default                                                                                           |
