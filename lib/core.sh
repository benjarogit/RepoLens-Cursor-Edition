...
validate_agent() {
  ...
  case "$agent" in
    ...
    spark/*|sparc/*)
      ...
      ;;
    *) die "Invalid agent: $agent (expected claude, codex, spark/sparc, cursor, cursor-ide, opencode, antigravity, or <agent>/<model> for claude/codex/opencode/antigravity)" ;;
  esac
}
...
require_agent_cmd() {
  ...
  case "$agent" in
    ...
    cursor-ide) ;; # Composer handoff — no CLI binary
...
resolve_agent_timeout() {
  ...
  case "$agent" in
    ...
    cursor|cursor-ide) agent_vars=(REPOLENS_AGENT_TIMEOUT_CURSOR) ;;
    "") ;;
    *) ;;
  esac
