...
# Tests for issue #95 - Android exported component attack surface lens.
#
# Behavioural contract:
#   - android/exported-components exists and is registered in config/domains.json.
#   - The prompt covers the exported activity/service/receiver/provider and IPC
#     risks named in the issue, including sharedUserId trust-boundary cases.
...
LENS_FILE="$SCRIPT_DIR/prompts/lenses/android/exported-components.md"
...
echo ""
echo "=== Test Suite: Android exported component lens (issue #95) ==="
echo ""

echo "Test 1: lens file exists"
if [[ -f "$LENS_FILE" ]]; then
  record_pass "exported-components lens file exists"
else
  record_fail "exported-components lens file exists"
fi
...
assert_contains "id frontmatter" "id: exported-components" "$lens_content"
assert_contains "domain frontmatter" "domain: android" "$lens_content"
assert_contains "name frontmatter" "name: Exported Component Surface Auditor" "$lens_content"
assert_contains "role frontmatter" "role: Android Component Surface Specialist" "$lens_content"
...
assert_contains "registered android lens list includes exported-components" "exported-components" "$android_lenses"

echo ""
echo "Test 4: issue exported-component risks are covered"
for risk in \
  "android:exported" \
  "intent-filter" \
  "targetSdkVersion" \
  ...
  "android:permission" \
  "signature" \
  "android:sharedUserId"; do
  assert_contains "covers $risk" "$risk" "$lens_content"
done

echo ""
echo "Test 5: sharedUserId is tied to each exported component family"
for component in \
  "exported activities" \
  "exported services" \
  "exported receivers" \
  "exported providers"; do
  assert_contains "covers sharedUserId plus $component" "$component" "$lens_content"
done

echo ""
echo "Test 6: investigation commands use shell-safe runtime APK variable"
...
echo ""
echo "Test 7: decoded output uses private per-run scratch directory"
...
assert_not_contains "does not use fixed exported-components shared path" "/tmp/exported-components" "$lens_content"
