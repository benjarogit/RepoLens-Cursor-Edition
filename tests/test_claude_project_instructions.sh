...
assert_contains "headline has total lens count" "runs $total_lenses expert analysis agents" "$claude_content"

echo ""
echo "Test 2: CLAUDE.md prompt inventory count matches domains.json"
assert_contains "prompt inventory has total lens count" "($total_lenses expert prompts)" "$claude_content"

echo ""
echo "Test 3: CLAUDE.md category breakdown matches domains.json"
assert_eq "category breakdown sums to total" "$total_lenses" "$breakdown_total"
