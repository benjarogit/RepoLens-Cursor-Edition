...
write_response_body() {
  ...
  local response response_tmp a1 a2 a3
  response="$(jq -r '.files.response' "$request_file")"
  response_tmp="${response}.tmp"
  # Default path cites three in-bounds lines (fixture has ≥3 lines) so DONE
  # streak proof (≥3 verified anchors) and substance checks both pass.
  a1="$anchor"
  a2="src/app.sh:2"
  a3="src/app.sh:3"
  if [[ "$anchor" == src/app.sh:3 ]]; then
    # Out-of-bounds test: only cite the invalid line (plus another invalid).
    a1="src/app.sh:3"
    a2="src/app.sh:4"
    a3="src/app.sh:5"
  elif [[ "$anchor" == linked-evidence/* ]]; then
    a1="$anchor"
    a2="linked-evidence/evidence.md:2"
    a3="linked-evidence/evidence.md:3"
  fi

  {
    printf '%s\n' "## Method"
    printf 'Inspected %s, %s, and %s and traced the only command from its entry point through the local fixture.\n' "$a1" "$a2" "$a3"
    ...
    printf '%s\n' "This evidence is specific to the current prompt and project state under the cited lines."
    printf '\n%s\n' "DONE"
  } > "$response_tmp"
...
mkdir -p "$project/src"
printf '%s\n' '#!/usr/bin/env bash' 'printf "%s\\n" safe' '# fixture pad' > "$project/src/app.sh"
...
assert_contains "out-of-bounds rejection explains the line-bound requirement" \
  ...
  "$(jq -r 'select(.kind == "error" and .code == "IDE_RESPONSE_REJECTED") | .reason' "$ctl_log" | tail -1)"
...
assert_eq "protocol records all rejected markers" "4" \
  "$(jq -s '[.[] | select(.kind == "error" and .code == "IDE_RESPONSE_REJECTED")] | length' "$ctl_log")"
assert_eq "protocol records one accepted response" "1" \
  "$(jq -s '[.[] | select(.kind == "ide_handoff_ok")] | length' "$ctl_log")"
...
assert_contains "non-local error distinguishes the Cursor CLI backend" \
  "currently support only --local mode" "$nonlocal_out"
...
assert_eq "control stream terminates with run_complete" "run_complete" \
  "$(jq -r 'select(.kind == "run_complete") | .kind' "$SCRIPT_DIR/logs/$e2e_run_id/cursor-ide/events.ndjson" | tail -1)"
assert_eq "run_complete asks for plan_mode" "plan_mode" \
  "$(jq -r 'select(.kind == "run_complete") | .next_action' "$SCRIPT_DIR/logs/$e2e_run_id/cursor-ide/events.ndjson" | tail -1)"
