...
assert_contains "logs dry-run lists race-conditions" "logs/race-conditions" "$logs_dry_output"
...
assert_contains "logs run completes race-conditions" "[logs/race-conditions] DONE x3" "$logs_run_output"
...
echo ""
echo "Test 7b: Domain-qualified focus can select duplicate logs lens IDs"
logs_focus_output="$(run_repolens "${RUN_PREFIX}-logs-focus-race" --domain logs --focus race-conditions --dry-run)"
logs_focus_rc=$?
assert_exit_code "logs race-conditions focus exits zero" 0 "$logs_focus_rc"
assert_contains "domain-qualified focus selects logs race lens" "logs/race-conditions" "$logs_focus_output"
assert_not_contains "domain-qualified focus excludes concurrency race lens" "concurrency/race-conditions" "$logs_focus_output"
...
assert_eq "logs domain registers expected lenses" "error-storms,error-cascades,retry-loops,recursive-growth,resource-leaks,resource-exhaustion,log-gaps,missing-heartbeats,silent-failures,state-machine-violations,state-corruption,race-conditions,lifecycle-violations,orphaned-events,process-orphans,latency-degradation,clock-skew,timeout-clusters,deadlock-symptoms,data-loss-signals,transaction-anomalies" "$logs_lens_ids"
